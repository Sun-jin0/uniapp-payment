...
async function importStructure() {
  ...
  try {
    ...
    // 读取JSON文件
    const jsonData = JSON.parse(await fs.readFile('f:/Code/uniapp/demo/结构/conputer_question_bank.json', 'utf8'));
        ...
        for (const [chapterId, chapterData] of Object.entries(chapters)) {
          ...
          if (examGroups) {
            for (const [examGroupId, examGroupData] of Object.entries(examGroups)) {
              ...
              try {
                await connection.query(
                  `INSERT INTO computer1_knowledge_tag (tag_id, tag_name, chapter_id, major_id, sort, total_number) 
                   VALUES (?, ?, ?, ?, ?, ?)
                   ON DUPLICATE KEY UPDATE 
                   tag_name = VALUES(tag_name),
                   sort = VALUES(sort),
                   total_number = VALUES(total_number)`,
                  [examGroupId, examGroupName, chapterId, majorId, examSort || 0, totalNumber || 0]
                );
